...
def Do(red_param, comp_param):
    ...
    ## test computation of V
    visual = False
    if visual:
        tmp = GridFunction(fes)
        Draw(tmp)
        for j in range(0, dim_red):
            tmp.vec.FV().NumPy()[:] = V[:, j]
            no = sqrt(Integrate(tmp*tmp, mesh))
            Redraw()
...
if __name__ == "__main__":
    ...
    param = np.arange(3,20, stepsize)
    test_param = np.arange(3,20, reduced_stepsize)
    sol_red = test_param
    # sol_orig = NormOfSolutions(param)
    [sol_orig, sol_red] = Do(param, test_param)
